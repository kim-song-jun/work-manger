 ...
 * Covers:
 *   - F-EMPLOYEE-009: 화면 진입 시 markAllRead 단일 호출 (N+1 패턴 방지)
 *   - 항목이 빈 경우 empty 상태 표시
 * Out of scope:
...
const markAllRead = notifApi.markAllRead as unknown as ReturnType<typeof vi.fn>;
...
describe("m-notifications page", () => {
  it("F-EMPLOYEE-009: calls markAllRead once (not N individual markRead calls) when unread items load", async () => {
    ...
    await waitFor(() => {
      // Single bulk call — not individual markRead per item
