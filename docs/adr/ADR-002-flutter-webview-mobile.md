...
- 네이티브 권한이 필요한 기능: GPS / Geofencing, 푸시 (ntfy + APNs direct — [ADR-006](./ADR-006-self-hosted-push-no-firebase.md)), 햅틱, 카메라(향후), 홈 화면 위젯
...
- **셸이 얇다**: `apps/mobile/` 는 진입점, 푸시 클라이언트 (ntfy/APNs — ADR-006), WebView 호스팅, 브리지, 자동 업데이트 호출만 담는다. 비즈니스 화면은 SPA 가 책임.
...
- 푸시 init 은 진입점에서 1회: Android 는 ntfy WebSocket (`apps/mobile/lib/notif/ntfy_client.dart`), iOS 는 표준 `UIApplication` 델리게이트 (`apps/mobile/ios/Runner/AppDelegate.swift`). Firebase 사용 안 함 — [ADR-006](./ADR-006-self-hosted-push-no-firebase.md).
...
- **단점**:
  ...
  - Flutter 가 백그라운드 작업 / 위젯 플러그인 생태계에서 최근 더 활발(2025~2026 기준). 푸시는 표준 OS API (APNs / WebSocket) 만 쓰므로 RN/Flutter 차이 무관 — [ADR-006](./ADR-006-self-hosted-push-no-firebase.md).
...
- **Flutter 의존성 업그레이드**: `flutter_inappwebview` 와 `web_socket_channel` 업그레이드 시 양 OS 회귀 테스트 필요. 분기별 정기 업그레이드 권장. (Firebase 의존성은 [ADR-006](./ADR-006-self-hosted-push-no-firebase.md) 으로 제거.)
...
- 푸시 정확도 / 도달율 모니터링: APNs 직접 응답 (BadDeviceToken 등) + ntfy publish HTTP status + 자체 ack receipt(앱이 푸시 수신 시 서버에 보고) 추가. 운영 가시성. ([ADR-006](./ADR-006-self-hosted-push-no-firebase.md))
- App Store / Play Store 심사 절차: [SOP-app-store-emergency-update](../operations/sop/sop-app-store-emergency-update.md) 참조.
- ~~빌드 산출물(`google-services.json`, `GoogleService-Info.plist`, `firebase_options.dart`) 은 git 에 커밋하지 않고 CI secrets 으로 주입. 환경별(dev/stg/prod) 로 별도 Firebase 프로젝트.~~ **2026-05 무효** — [ADR-006](./ADR-006-self-hosted-push-no-firebase.md) 에서 Firebase 의존성을 완전히 제거. 푸시는 Web Push (VAPID) + APNs HTTP/2 직접 + ntfy (self-hosted) 로 라우팅. `firebase_*` Flutter 패키지, `google-services.json`, `GoogleService-Info.plist`, `firebase_options.dart` 모두 더 이상 사용하지 않는다.
